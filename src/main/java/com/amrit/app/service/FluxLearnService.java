...
public class FluxLearnService {
    ...
    public Flux<String> getFlux() {
        return Flux.just("Abinash", "Agya", "Amrit", "Aneel");
    }

    public Flux<String> fruitsFlux() {
        List<String> fruitsNames = List.of("Apple", "Apple");
...
//                .defaultIfEmpty("Learn Code With Durgesh")
                .switchIfEmpty(fruitsFlux()).log();
    }

    //    concat(static) / concatWith(instance)
    public Flux<String> concatExample() {
        return Flux.concat(
                getFlux().delayElements(Duration.ofSeconds(1)),
                fruitsFlux().delayElements(Duration.ofSeconds(2)));
